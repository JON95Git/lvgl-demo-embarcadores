...
static _gps_parse_err_t gps_parse_buffer_to_search_string(char *string);
...
void gps_callback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == GPS_USART_INSTANCE) {
        gps_stop_dma();
        if (gps_parse_buffer_to_search_string("$GPRMC") == GPS_PARSE_ERR_SUCESS) {
            osSignalSet(gpsTaskHandle, GPS_TASK_SIGNAL);
            return;
        } 
        if (gps_parse_buffer_to_search_string("$GPGGA") == GPS_PARSE_ERR_SUCESS) {
            osSignalSet(gpsTaskHandle, GPS_TASK_SIGNAL);
            return;
        } else {
            gps_start_dma();
...
static _gps_parse_err_t gps_parse_buffer_to_search_string(char *string)
{
    ...
    if (gps_buffer_ptr != NULL) {
        ...
        gps_print_buffer();
#endif
        return GPS_PARSE_ERR_SUCESS;
    } else {
        return GPS_PARSE_ERR_FAILED;
    }
}
